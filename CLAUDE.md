...
- `src/lib/config.ts` — Domain constants: `SESSION_TOPICS`, `SESSION_OUTCOMES`, `NEXT_STEPS_OPTIONS`, `ENGAGEMENT_LEVEL_OPTIONS`, `ACTION_COMMITMENT_OPTIONS`, `NUDGE_THRESHOLDS`, `PROGRAM_TRACK_SESSIONS`, and `PROGRAM_ADMIN` (single source of truth for participant-facing admin contact).
