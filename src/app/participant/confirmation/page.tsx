...
function ConfirmationContent() {
  ...
  // Error state — no coach data and not an already-selected redirect
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6">
        <div className="w-full max-w-sm text-center">
          ...
          />
          <h1 className="font-display text-2xl font-light text-fc-950">
            Something went wrong
          </h1>
          <p className="mt-3 text-sm text-muted-foreground">
            We could not load your confirmation. Please start over.
          </p>
          <Button
            className="mt-6"
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex flex-1 items-start justify-center px-6 py-16">
        <div className="w-full max-w-md">
          ...
          >
            <h1 className="font-display text-3xl font-light leading-tight text-fc-950 sm:text-4xl">
              {alreadySelected && !coach
                ...
                : "Your Coach Has Been Selected"}
            </h1>
            {alreadySelected && !coach && (
              <p className="mt-3 text-sm text-muted-foreground">
                You have already selected a coach for this program. Check your
                email for details or contact your program administrator.
              </p>
            )}
          </div>

          {/* Coach card — shown when we have coach data */}
          {coach && (
            <div
            ...
            >
              ...
              <div className="mb-6 flex flex-col items-center gap-3 rounded-2xl border border-fc-100 bg-white p-6 text-center">
                ...
                <div>
                  <p className="font-display text-xl font-medium text-fc-900">
                    {coach.name}
                  </p>
              ...
              {coach.bookingUrl ? (
                <div className="space-y-3 text-center">
                  <Button
                    size="lg"
                    className="w-full"
                    onClick={() => window.open(coach.bookingUrl, "_blank")}
                  >
                    Book your first session
                  </Button>
                  <p className="text-sm text-muted-foreground">
                    Opens your coach&apos;s scheduling page in a new tab
                  </p>
                </div>
              ) : (
                <div className="rounded-xl border border-fc-200 bg-fc-50 p-5 text-center">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Your coach will reach out within 2 business days to schedule
                    your first session
                  </p>
                </div>
              )}
          ...
          {/* Footer */}
          <p
            className={`mt-10 text-center text-xs text-muted-foreground transition-all duration-700 ${
...
// Outer page — wraps content in Suspense for useSearchParams compatibility
